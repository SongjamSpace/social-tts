...
export async function GET() {
  try {
    const apiUrl = `https://songjamspace-leaderboard.logesh-063.workers.dev/pumpfun_followers`;
    ...
    if (!res.ok) {
      throw new Error(`Followers API responded with status: ${res.status}`);
    }
    ...
    console.error("Error fetching followers creators:", error);

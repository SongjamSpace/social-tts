...
export async function GET() {
  try {
    const apiUrl = `https://songjamspace-leaderboard.logesh-063.workers.dev/pumpfun_bonded`;
    ...
    if (!res.ok) {
      throw new Error(`Bonded API responded with status: ${res.status}`);
    }
    ...
    console.error("Error fetching bonded creators:", error);

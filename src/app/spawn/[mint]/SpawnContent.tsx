...
import { getPrepaidAgent } from "@/lib/prepaid-agents";
...
export default function SpawnContent({
  ...
  const prepaidAgent = mint ? getPrepaidAgent(mint) : undefined;

  // Auto-select size for pre-paid agents (must be before early returns to satisfy Rules of Hooks)
  useEffect(() => {
    if (prepaidAgent && !selectedSize) {
      setSelectedSize(prepaidAgent.size);
    }
  }, [prepaidAgent, selectedSize]);
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-[#060608] text-white px-4 py-8">
      <div className="max-w-md mx-auto">
        ...
        {error && <p className="text-red-400 text-sm mb-4">{String(error)}</p>}

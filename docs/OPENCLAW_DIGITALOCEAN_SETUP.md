...
This guide covers the environment variables and DigitalOcean setup needed so the **hatch** flow can deploy OpenClaw agents on App Platform.
...
|----------|----------|-------------|
| `DIGITALOCEAN_TOKEN` | **Yes** (for real deploys) | DigitalOcean API token with read/write access to create and manage App Platform apps. |
...
| `DIGITALOCEAN_APP_REGION` | No | App Platform region (default: `nyc`). Examples: `nyc`, `sfo`, `ams`, `sgp`. |
...
**Official OpenClaw stack (Control UI, onboarding, channels, skills):**

```bash
# .env.local – full OpenClaw Gateway + Control UI
...
The hatch API automatically sets `OPENCLAW_HOST=0.0.0.0` in the container when using port 18789 so the Gateway listens on all interfaces and passes DigitalOcean health checks.
...
- [ ] (Optional) `DIGITALOCEAN_APP_REGION` set if not using `nyc`.  
...
After this, when a user completes the hatch payment and onboarding, `POST /api/openclaw/hatch` will create an App Platform app from your image and store the app URL (and gateway token for official OpenClaw) in Firestore. Show the user the **gateway token** once (e.g. on the success or “Open agent” screen) so they can paste it into the Control UI when prompted. The “Open agent” link then opens the agent’s Control UI (or minimal chat, depending on the image).
